...
abstract class AbstractHcSlave extends AbstractSlave
{
    ...
    abstract protected function getEventDescriberClassName(): string;
    ...
    public function writeAddress(Module $slave, int $address): void
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_ADDRESS, ['slave' => $slave, 'newAddress' => $address]);
    ...
    public function readDeviceId(Module $slave): int
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_DEVICE_ID, ['slave' => $slave, 'deviceId' => $deviceId]);
    ...
    public function readTypeId(Module $slave): int
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_TYPE_ID, ['slave' => $slave, 'typeId' => $typeId]);
    ...
    public function writeTypeId(Module $slave, Type $type): AbstractSlave
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_TYPE_ID, ['slave' => $slave, 'typeId' => $type->getId()]);

        $this->write($slave, self::COMMAND_TYPE, chr((int) $type->getId()));

        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_TYPE_ID, ['slave' => $slave, 'typeId' => $type->getId()]);
    ...
    {
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_PWM_SPEED, ['slave' => $slave, 'pwmSpeed' => $speed]);
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_PWM_SPEED, ['slave' => $slave, 'pwmSpeed' => $speed]);
    }
    ...
    protected function readConfig(Module $slave, int $length): string
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_CONFIG, ['slave' => $slave, 'config' => $config]);
    ...
    public function readHertz(Module $slave): int
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_HERTZ, ['slave' => $slave, 'hertz' => $hertz]);
    ...
    public function readPwmSpeed(Module $slave): int
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_PWM_SPEED, ['slave' => $slave, 'pwmSpeed' => $speed]);
    ...
    public function readEepromSize(Module $slave): int
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_EEPROM_SIZE, ['slave' => $slave, 'eepromSize' => $eepromSize]);
    ...
    public function readEepromFree(Module $slave): int
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_EEPROM_FREE, ['slave' => $slave, 'eepromFree' => $eepromFree]);
    ...
    public function readEepromPosition(Module $slave): int
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_EEPROM_POSITION, ['slave' => $slave, 'eepromPosition' => $eepromPosition]);
    ...
    {
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_EEPROM_POSITION, ['slave' => $slave, 'eepromPosition' => $position]);
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_EEPROM_POSITION, ['slave' => $slave, 'eepromPosition' => $position]);
    }
    ...
    public function readBufferSize(Module $slave): int
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_BUFFER_SIZE, ['slave' => $slave, 'bufferSize' => $bufferSize]);
    ...
    public function readLedStatus(Module $slave): array
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_LED_STATUS, $eventData);
    ...
    {
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_POWER_LED, ['slave' => $slave, 'on' => $on]);

        $this->write($slave, self::COMMAND_POWER_LED, chr((int) $on));

        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_POWER_LED, ['slave' => $slave, 'on' => $on]);
    }
    ...
    {
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_ERROR_LED, ['slave' => $slave, 'on' => $on]);

        $this->write($slave, self::COMMAND_ERROR_LED, chr((int) $on));

        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_ERROR_LED, ['slave' => $slave, 'on' => $on]);
    }
    ...
    {
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_CONNECT_LED, ['slave' => $slave, 'on' => $on]);

        $this->write($slave, self::COMMAND_CONNECT_LED, chr((int) $on));

        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_CONNECT_LED, ['slave' => $slave, 'on' => $on]);
    }
    ...
    {
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_TRANSRECEIVE_LED, ['slave' => $slave, 'on' => $on]);

        $this->write($slave, self::COMMAND_TRANSRECEIVE_LED, chr((int) $on));

        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_TRANSRECEIVE_LED, ['slave' => $slave, 'on' => $on]);
    }
    ...
    {
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_TRANSCEIVE_LED, ['slave' => $slave, 'on' => $on]);

        $this->write($slave, self::COMMAND_TRANSCEIVE_LED, chr((int) $on));

        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_TRANSCEIVE_LED, ['slave' => $slave, 'on' => $on]);
    }
    ...
    {
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_RECEIVE_LED, ['slave' => $slave, 'on' => $on]);

        $this->write($slave, self::COMMAND_RECEIVE_LED, chr((int) $on));

        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_RECEIVE_LED, ['slave' => $slave, 'on' => $on]);
    }
    ...
    {
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_CUSTOM_LED, ['slave' => $slave, 'on' => $on]);

        $this->write($slave, self::COMMAND_CUSTOM_LED, chr((int) $on));

        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_CUSTOM_LED, ['slave' => $slave, 'on' => $on]);
    }
    ...
    public function readPowerLed(Module $slave): bool
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_POWER_LED, ['slave' => $slave, 'on' => $on]);
    ...
    public function readErrorLed(Module $slave): bool
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_ERROR_LED, ['slave' => $slave, 'on' => $on]);
    ...
    public function readConnectLed(Module $slave): bool
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_CONNECT_LED, ['slave' => $slave, 'on' => $on]);
    ...
    public function readTransreceiveLed(Module $slave): bool
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_TRANSRECEIVE_LED, ['slave' => $slave, 'on' => $on]);
    ...
    public function readTransceiveLed(Module $slave): bool
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_TRANSCEIVE_LED, ['slave' => $slave, 'on' => $on]);
    ...
    public function readReceiveLed(Module $slave): bool
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_RECEIVE_LED, ['slave' => $slave, 'on' => $on]);
    ...
    public function readCustomLed(Module $slave): bool
    {
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_CUSTOM_LED, ['slave' => $slave, 'on' => $on]);
    ...
    public function writeRgbLed(
        ...
        $eventData = $colors;
        $eventData['slave'] = $slave;
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_RGB_LED, $eventData);
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_RGB_LED, $eventData);
    }
    ...
    public function readRgbLed(Module $slave): array
    {
        ...
        $eventData = $colors;
        $eventData['slave'] = $slave;
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::READ_RGB_LED, $eventData);
    ...
    public function writeAllLeds(
        ...
        $eventData = $leds;
        $eventData['slave'] = $slave;
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::BEFORE_WRITE_ALL_LEDS, $eventData);
        $this->write(
            $slave,
        ...
        $this->eventService->fire($this->getEventDescriberClassName(), AbstractHcDescriber::AFTER_WRITE_ALL_LEDS, $eventData);
    }
