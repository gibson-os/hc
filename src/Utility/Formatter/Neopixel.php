...
class Neopixel extends AbstractHcFormatter
{
     ...
     * @return string[]
     */
    public static function getLedsAsStrings(array $leds, int $maxLength): array
    {
        ksort($leds);
     ...
     * @return array
     */
    public static function getLedsAsArray(string $data): array
    {
        $leds = [];
            ...
            if ($address > self::MAX_PROTOCOL_LEDS) {
                for ($j = 0; $j < $address - self::MAX_PROTOCOL_LEDS; $j++) {
                    ...
                    $leds[$addressFromGroup] = [
                        ...
                        LedAttribute::ATTRIBUTE_KEY_FADE_IN => Transform::asciiToInt($data >> 4, $i++),
                        LedAttribute::ATTRIBUTE_KEY_BLINK => Transform::asciiToInt($data & 15, $i++),
                    ];
                }
            }
     ...
     * @return string
     */
    private static function getRangedColorString(array &$leds, array &$color): string
    {
        ...
        for ($i = $firstLed; $i <= end($color['leds']); $i++) {
            ...
            unset($color['leds'][$colorLedIndex]);
            unset($leds[$i]);
            $rangedLeds[] = $i;
        }
     ...
     * @return string
     */
    private static function getSingleColorString(array $color): string
    {
     ...
     * @return string[]
     */
    private static function getGroupedColorStrings(array $color, int $maxLength): array
    {
    ...
    private static function completeGroupedColorString(string $data, int $count, array $color): string
    {
     ...
     * @return array
     */
    private static function getColorsByLeds(array $leds): array
    {
        $colors = [];
