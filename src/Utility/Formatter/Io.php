...
class Io extends AbstractHcFormatter
{
    /**
     * @var int
     */
    private static $directConnectReadInputPort;
    ...
    public function text(): ?string
    {
        switch ($this->command) {
            case IoService::COMMAND_CONFIGURATION:
                return 'Port Anzahl: ' . Transform::hexToInt($this->data);
            case IoService::COMMAND_DEFRAGMENT_DIRECT_CONNECT:
                return 'Defragmentieren';
            case IoService::COMMAND_READ_DIRECT_CONNECT:
                if ($this->direction == Server::DIRECTION_OUTPUT) {
                    return null;
                }

                $lastByte = Transform::hexToInt($this->data, 2);
                ...
                return null;
            case IoService::COMMAND_DIRECT_CONNECT_STATUS:
                return Transform::hexToInt($this->data, 0) ? 'Aktiv' : 'Inaktiv';
            case IoService::COMMAND_STATUS_IN_EEPROM:
                if ($this->direction == Server::DIRECTION_OUTPUT) {
                    return 'Standard gesetzt';
                }

                if (Transform::hexToInt($this->data, 0)) {
    ...
    public function render(): ?string
    {
        switch ($this->command) {
            ...
            case IoService::COMMAND_DATA_CHANGED:
                ...
                return $return . '</table>';
            case IoService::COMMAND_ADD_DIRECT_CONNECT:
                $inputPort = Transform::hexToInt($this->data, 0);
                ...
                return
                    '<table>' .
                        ...
                        '</tr>' .
                        $this->getDirectConnectTableRows($inputPort, substr(Transform::hexToAscii($this->data), 1)) .
                    '</table>';
            case IoService::COMMAND_SET_DIRECT_CONNECT:
                $inputPort = Transform::hexToInt($this->data, 0);
                $inputName = Value::getByTypeId(
                ...
                return
                    '<table>' .
                        ...
                        '<tr>' .
                            '<th>Nummer</th>' .
                            '<td>' . Transform::hexToInt($this->data, 1) . '</td>' .
                        '</tr>' .
                    $this->getDirectConnectTableRows($inputPort, substr(Transform::hexToAscii($this->data), 2)) .
                    '</table>';
            case IoService::COMMAND_DELETE_DIRECT_CONNECT:
                $inputPort = Transform::hexToInt($this->data, 0);
                $inputName = Value::getByTypeId(
            ...
            case IoService::COMMAND_READ_DIRECT_CONNECT:
                if ($this->direction == Server::DIRECTION_OUTPUT) {
                    self::$directConnectReadInputPort = Transform::hexToInt($this->data, 0);
                    $inputName = Value::getByTypeId(
                        $this->module->getTypeId(),
                ...
                return
                    '<table>' .
                        $this->getDirectConnectTableRows(
                            self::$directConnectReadInputPort,
                            Transform::hexToAscii($this->data)
                        ) .
                    '</table>';
            case IoService::COMMAND_STATUS_IN_EEPROM:
                //return 'Status in EEPROM';
        ...
        if ($this->command < $this->module->getConfig()) {
            $port = self::getPortAsArray(Transform::hexToAscii($this->data));
            $valueNames = Value::getByTypeId(
                $this->module->getTypeId(),
                $this->command,
                $this->module->getId(),
    ...
    {
        $ports = self::getPortsAsArray(Transform::hexToAscii($this->data), $this->module->getConfig());
        ...
        $lastPorts = self::getPortsAsArray(Transform::hexToAscii($lastData), $this->module->getConfig());
        $changedPorts = [];
    ...
    private function getDirectConnectTableRows(int $inputPort, string $data): string
    {
        ...
        $outputName = Value::getByTypeId(
            ...
            $this->module->getId(),
            IoService::ATTRIBUTE_TYPE_PORT, IoService::ATTRIBUTE_PORT_KEY_NAME
        );
        ...
        if ($directConnect[IoService::ATTRIBUTE_DIRECT_CONNECT_KEY_ADD_OR_SUB] == 1) {
            $addOrSub = 'Addieren';
        } else if ($directConnect[IoService::ATTRIBUTE_DIRECT_CONNECT_KEY_ADD_OR_SUB] == -1) {
     ...
     * @return array
     */
    public static function getPortAsArray(string $data): array
    {
        $byte1 = Transform::asciiToInt($data, 0);
        $byte2 = Transform::asciiToInt($data, 1);
     ...
     * @return array
     */
    public static function getPortsAsArray(string $data, int $portCount): array
    {
        $byteCount = 0;
        $ports = [];
     ...
     * @return string
     */
    public static function getPortAsString(array $data): string
    {
     ...
     * @return array
     */
    public static function getDirectConnectAsArray(string $data): array
    {
        $inputValueAndOutputPortByte = Transform::asciiToInt($data, 0);
        $setByte = Transform::asciiToInt($data, 1);
        $pwmByte = Transform::asciiToInt($data, 2);
        $addOrSub = 0;
     ...
     * @return string
     */
    public static function getDirectConnectAsString(
        int $inputPort,
        int $inputValue,
        int $outputPort,
        int $value,
